...
| FR-41 | Sidebar toggle button (hamburger) is fixed to the top-left of the viewport, outside the calculator card, with `aria-label` and `aria-expanded`; it animates into an X when open. | `index.html` / `components.css` |
