...
| `components.css` | Glass card (`.calculator`), display (`.result`/`.history`), theme button (`.theme-btn`), viewport/sidebar-header toggle (`.sidebar-toggle`), sidebar (`.sidebar`), button grid (`.btn*`), projects page (`.projects-*`), modal (`.modal-*`), focus-visible styles, and keyboard-sync `.btn-pressed` state. |
